...
st.markdown("""
    ...
    [data-testid="stCameraInput"] { height: 65vh !important; min-height: 500px !important; }
    [data-testid="stCameraInput"] > div { height: 100% !important; }
    [data-testid="stCameraInput"] video { height: 100% !important; object-fit: cover !important; }
    </style>
...
# FIX 2: State management for the Post-Audit Success Screen
...
# ==========================================
# FIX 3: Corrected Logo Filename & Fallback Alert
if os.path.exists("asiauto_logo.png"): 
    st.sidebar.image("asiauto_logo.png", use_container_width=True)
else:
    st.sidebar.caption("*(Logo file 'asiauto_logo.png' no encontrado en el servidor)*")
...
if menu == "📊 Dashboard Global":
    ...
    # FIX 4: Rearranged Dashboard Layout (No Columns, 100% Width, Specific Order)
    if total_audits > 0:
        
        # CHART 1: Cumplimiento por Categoría Evaluada
        ...
        if session_ids:
            ...
            if records:
                ...
                fig_bar.update_layout(height=400, margin=dict(l=20, r=20, t=20, b=20), xaxis_title="Cumplimiento (%)", yaxis_title="")
                fig_bar.update_xaxes(range=[0, 115])
                st.plotly_chart(fig_bar, use_container_width=True)

        st.markdown("<hr class='slim'>", unsafe_allow_html=True)

        # CHART 2: Score Global (Gauge)
        ...
        # CHART 3: Evolución de Cumplimiento
        ...
        fig_trend.update_layout(height=350, margin=dict(l=20, r=20, t=30, b=20), yaxis_title="Score (%)", xaxis_title="")
    ...
    if sel_ag != "Sin resultados":
        ...
        if agency_sessions:
            ...
            filtro_vista = st.radio(
                ...
                ["Predeterminado (Ocultar evidencia de SI)", "Mostrar Todo (Incluir evidencia de SI)", "Solo Fallas (NO)"], 
                horizontal=True
            )
            ...
            for r in records:
                ...
                mostrar_evidencia = (not is_pass) or (is_pass and filtro_vista == "Mostrar Todo (Incluir evidencia de SI)")
...
# ==========================================
    ...
    # FIX 2: Success Screen Implementation
    if st.session_state['audit_success']:
        st.success("✅ ¡Auditoría Finalizada y Guardada con Éxito!")
        st.metric("Score Final Obtenido", f"{st.session_state['last_score']:.1f}%")
        st.info("Los resultados ya están disponibles en el Dashboard Global y en su Historial.")
        
        if st.button("Finalizar y Volver al Menú Principal", type="primary", use_container_width=True):
            st.session_state['audit_success'] = False
            st.rerun()
        ...
        if in_progress:
            ...
            if st.session_state.get('active_session_id') == session_id:
                ...
                if pending_items:
                    ...
                    # FIX 1: Move Toggle OUTSIDE the form to ensure state refreshes immediately
                    st.markdown("**Método de Evidencia Fotográfica:**")
                    captura_modo = st.radio("Seleccione el origen de la foto:", ["Cámara en Vivo (Rápido)", "Subir Galería"], horizontal=True, key=f"mode_{current_item['id']}", label_visibility="collapsed")
                    ...
                    with st.form("single_question_form", clear_on_submit=True):
                        ...
                        # Render the correct uploader based on the toggle chosen outside the form
                        if captura_modo == "Cámara en Vivo (Rápido)":
                            uploaded_photo = st.camera_input("Tome la foto de la falla", key=f"cam_{current_item['id']}")
                        else:
                            uploaded_photo = st.file_uploader("Seleccione de la galería", type=['jpg', 'jpeg', 'png'], key=f"up_{current_item['id']}")
                    ...
                    if st.button("🏁 Cerrar Auditoría y Calcular Score Final", type="primary", use_container_width=True):
                        ...
                        # FIX 2: Trigger the Success Screen state instead of resetting silently
            ...
            for plan in plans:
                ...
                with st.expander(f"🔴 [{cat['item_code']}] {cat['audit_question'][:60]}..."):
                    ...
                    if rec_data.get('failure_photo_url'):
                        st.image(rec_data['failure_photo_url'], caption="Evidencia de la falla reportada", width=400)
