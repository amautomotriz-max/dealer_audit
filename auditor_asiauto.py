        ...
        if st.session_state.get('active_session_id') == session_id:
            ...
            if pending_items:
                ...
                with st.form("single_question_form", clear_on_submit=True):
                    ...
                    st.info("📸 **Evidencia Fotográfica** (Opcional en SI, Obligatorio en NO)")
                    
                    # Switch para permitir captura en vivo ultraligera o subida de galería
                    captura_modo = st.radio("Método de captura:", ["Cámara en Vivo (Rápido)", "Subir Galería"], horizontal=True, key=f"mode_{current_item['id']}")
                        ...
                        if not is_pass and uploaded_photo is None:
                            ...
                            with st.spinner('Procesando datos y subiendo a Supabase...'):
                                ...
                                if uploaded_photo is not None:
                                    ...
                                    try:
                                        # Compresión en el servidor (muy rápida porque la entrada de la cámara web ya es ligera)
