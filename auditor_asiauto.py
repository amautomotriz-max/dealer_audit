...
if menu == "📊 Dashboard Global":
            ...
            with c_edit2:
                ...
                if st.button("🗑️ Eliminar Ítem", type="primary", use_container_width=True):
                    try:
                        ...
                        st.error("No se puede eliminar (ya tiene historial).")
        ...
        if sessions:
            ...
            for r in records:
                ...
                if not r['result_pass']:
                    st.caption(f"Comentario: {r['auditor_comment']}")
    ...
    if not pending: st.success("Todo al día.")
        ...
        for plan in pending:
            st.markdown(f"### 📍 {plan['audit_records']['audit_sessions']['audit_agencies']['name']}")
            c1, c2 = st.columns(2)
            with c1:
...
# ==========================================
# 6. VIEWS: AUDITOR (INCLUDES RIGOR FILTER)
# ==========================================
    ...
    if in_progress:
        ...
        # Recuperar el rigor guardado en la sesión (se asume guardado en campo metadata o similar, 
        # o se usa el rigor del catálogo filtrado por los ya respondidos)
        ...
        if st.session_state.get('active_session_id') == session_id:
            ...
            # Recuperar el rigor seleccionado de la sesión (usamos rigor_level_filter guardado en audit_sessions)
            ...
            if pending_items:
                ...
                st.info(f"**Pregunta:** {current_item['audit_question']}")
                
                with st.form("single_question_form", clear_on_submit=True):
                    res = st.radio("Resultado:", ["SI", "NO"], horizontal=True)
                    com = st.text_input("Comentario (Obligatorio en NO)")
                    captura_modo = st.radio("Cámara:", ["Vivo (WebRTC)", "Galería"], horizontal=True)
                    uploaded_photo = st.camera_input("Foto") if captura_modo == "Vivo (WebRTC)" else st.file_uploader("Subir")
        ...
        agencias = supabase.table("audit_agencies").select("id, name").eq("brand", marca).execute().data
        if agencias:
            ...
            if st.button("🚀 INICIAR AUDITORÍA", type="primary", use_container_width=True):
                new_session = supabase.table("audit_sessions").insert({
                    "agency_id": ag_dict[sel_ag], "auditor_id": st.session_state['user_id'], 
                    "marca": marca, "status": "EN PROCESO", "rigor_level_filter": rigor_sel
                }).execute()
    ...
    if records:
        ...
        plans = supabase.table("audit_action_plans").select("*").in_("record_id", rec_ids).eq("status", "🔴 ABIERTO").execute().data
        for plan in plans:
            with st.expander(f"Plan: {plan['id']}"):
                act = st.text_input("Acción realizada")
                pic = st.file_uploader("Evidencia")
                if st.form_submit_button("Enviar"):
                    # Lógica de subida y cierre similar a la anterior...
                    st.success("Plan enviado.")
